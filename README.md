...
* Download the JVC script file for your device architecture, and extract it in the same folder.
    ...
    * It is also possible to run them manually, using the ``` java <command> ``` standard tools provided by the JVM. In this case, remember that every Java dependency file is contained inside the **java** global folder.
